...
class AlertConsumerIntegrationTest {
    ...
    @Test
    void alertEventIsPersistedWithHighSeverity() throws Exception {
        UUID eventId = UUID.randomUUID();
        EventEnvelope envelope = new EventEnvelope(
                eventId, EventTypes.ALERT, Instant.now(), Instant.now(),
                "test-source", "corr-alert-1", null, "1");
        ...
        await().atMost(15, TimeUnit.SECONDS).untilAsserted(() ->
                assertThat(repository.findById(eventId))
                        ...
                        .get()
                        .satisfies(e -> {
                            assertThat(e.getEventType()).isEqualTo(EventTypes.ALERT);
                            assertThat(e.getSeverity()).isEqualTo("HIGH");
                            assertThat(e.getCorrelationId()).isEqualTo("corr-alert-1");
                        })
        );
    }

    @Test
    void nonAlertEventIsNotPersisted() throws Exception {
        UUID eventId = UUID.randomUUID();
        EventEnvelope envelope = new EventEnvelope(
                eventId, EventTypes.PRICING, Instant.now(), Instant.now(),
                "test-source", "corr-alert-2", null, "1");
